...
func newRequestShim(c *ginlib.Context, maxBodyBytes int64) *requestShim {
	if maxBodyBytes <= 0 {
		maxBodyBytes = DefaultMaxBodyBytes
	}
	shim := &requestShim{req: c.Request, maxBytes: maxBodyBytes}
	if c.Request.Body != nil {
		shim.origBody = c.Request.Body
		shim.source = c.Request.Body
		c.Request.Body = &replayBody{shim: shim}
	}
	if routeID, ok := c.Request.Context().Value(routeIDContextKey{}).(string); ok {
